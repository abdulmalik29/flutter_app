...
import 'package:budget/features/sms_parser/sms_reader.dart';
import 'package:flutter/widgets.dart';
import 'package:budget/struct/appKeys.dart';
...
class HomePageState extends State<HomePage> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void initState() {
    ...
    //     if (kDebugMode) print("HomePageState: Initial delayed _checkAndTriggerSmsScan call.");
    //     _checkAndTriggerSmsScan(isInitialLoad: true);
    //   }
    // });
  }
  ...
  //   super.didChangeAppLifecycleState(state); // Good practice to call super
  //   if (kDebugMode) print("HomePageState: didChangeAppLifecycleState - $state");
  //   if (state == AppLifecycleState.resumed) {
  //     if (kDebugMode) print("HomePageState: App Resumed. Checking for SMS re-scan.");
  //     _checkAndTriggerSmsScan();
  //   }
  // }

// Future<void> _checkAndTriggerSmsScan({bool isInitialLoad = false}) async {
//     if (kDebugMode) print("HomePageState: _checkAndTriggerSmsScan called (isInitialLoad: $isInitialLoad)");
//     if (!mounted) {
//       if (kDebugMode) print("HomePageState: _checkAndTriggerSmsScan - Widget not mounted. Exiting.");
//       return;
//     }

//     if (_isAutoScanningSms) {
//       if (kDebugMode) print("HomePageState: Auto-scan already in progress. Skipping.");
//       return;
//     }

//     final prefs = await SharedPreferences.getInstance();
//     final bool smsAutomationEnabled = prefs.getBool(kEnableSmsAutomationPrefKey) ?? false;
//     if (kDebugMode) print("HomePageState: SMS Automation Preference: $smsAutomationEnabled");

//     if (smsAutomationEnabled) {
//       if (kDebugMode) print("HomePageState: SMS Automation enabled. Triggering re-scan of latest messages.");
//       if (mounted) {
//         setState(() {
//           _isAutoScanningSms = true;
//         });
//       }

//       try {
//         int insertedCount = await readAndParseLatestSms(50, runInBackground: true);

//         if (kDebugMode) print("HomePageState: Automatic SMS re-scan complete. Inserted $insertedCount new transactions.");

//         if (insertedCount > 0 && mounted) {
//           refreshState();
//         }
//       } catch (e) {
//         if (kDebugMode) print("HomePageState: Error during automatic SMS re-scan: $e");
//       } finally {
//         if (mounted) {
//           setState(() {
//             _isAutoScanningSms = false;
//           });
//         }
//       }
//     } else {
//       if (kDebugMode) print("HomePageState: SMS Automation is disabled. No automatic re-scan.");
//     }
//   }
